export const SET_CUR_PROJ = 'SET_CUR_PROJ';
export const SET_CUR_USER = 'SET_CUR_USER';

export const SET_CUR_CREW = 'SET_CUR_CREW';

export const SET_WAGE_LIST = 'SET_WAGE_LIST';
export const SET_WAGE_INPUT = 'SET_WAGE_INPUT';

export const SET_FINE_LIST = 'SET_FINE_LIST';
export const DELETE_FINE = 'DELETE_FINE';
export const SET_FINE_INPUT = 'SET_FINE_INPUT';

export const SET_CREW_LIST = 'SET_CREW_LIST';
export const SET_CREW_INPUT = 'SET_CREW_INPUT';
export const DELETE_CREW = 'DELETE_CREW';

export const SET_APPLY_LIST = 'SET_APPLY_LIST';
export const DELETE_APPLY = 'DELETE_APPLY';
export const SET_PROJECT_LIST = 'SET_PROJECT_LIST';

export const SET_PAGE_CONF = 'SET_PAGE_CONF';
export const SET_PROJECT_OFFERS = 'SET_PROJECT_OFFERS';
export const SET_OFFER_PLUGINS = 'SET_OFFER_PLUGINS';
export const SET_PROJECT_POST = 'SET_PROJECT_POST';


